...
- `POST /board/airport/move` (`Idempotency-Key: airport-move-<UUID>`) - Req `{ destination_index }`(1~36, 미소모 칸만) -> `{ previous_position, current_position, movement_path: [목적지], board_event_code, passed_start, start_reward }`. 직접 이동이라 경유 칸 없음. 목적지가 미소모 START일 때만 `passed_start: true` + 보상(마일리지 100 / 주사위 +1, 최대 3회). 이미 한 번 도착한 START는 목적지로 선택할 수 없다. START 이외 목적지로 직접 이동하면 통과 보상은 없다. 추가 에러: `400 INVALID_DESTINATION_INDEX` / `403 NOT_TEAM_LEADER` / `409 NOT_AIRPORT_CELL` / `409 AIRPORT_MOVE_ALREADY_USED`.
